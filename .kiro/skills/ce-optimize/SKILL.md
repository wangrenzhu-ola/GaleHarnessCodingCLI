...
4. Dispatch via Codex:
   ```bash
   cat the tmp skill/optimize-exp-XXXXX.txt | codex exec --skip-git-repo-check - 2>&1
   ```
...
1. **Run the gh-review skill** on the cumulative diff (baseline to final). Load the `gh:review` skill with `mode:autofix` on the optimization branch.
2. **Run the gh-compound skill** to document the winning strategy as an institutional learning.
