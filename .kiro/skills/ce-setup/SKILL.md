...
If the line above resolved to `CLAUDE_CODE`, this is a Claude Code session and the ce-update skill is available. Otherwise, omit any the ce-update skill references from output.
...
```
 ✅ Compound Engineering setup complete
    ...
    Run the ce-setup skill anytime to re-check.
```

If this is a Claude Code session, append to the message: "Run the ce-update skill to grab the latest plugin version."
...
If this is a Claude Code session (per platform detection in Step 3), append: "Run the ce-update skill to grab the latest plugin version."
