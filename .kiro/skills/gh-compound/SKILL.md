...
```bash
the gh-compound skill                    # Document the most recent fix
the gh-compound skill [brief context]    # Provide additional context hint
```
...
<!-- the hkt-patch-gale-task-start skill -->
...
#### 4. **Session Historian** (foreground, after launching the above — only if the user opted in)
   ...
   - Dispatch in **foreground** — this agent reads session files outside the working directory (`~/.kiro/projects/`, `~/.codex/sessions/`, `~/.cursor/projects/`) which background agents may not have access to
...
5. After a successful HKTMemory store, log the memory_linked event (use the same `<frontmatter title>` as the `--memory-title` value):
   ...
   ```
<!-- the hkt-patch-gale-task-memory skill -->
...
- the gh-compound-refresh skill plugin-versioning-requirements`
- the gh-compound-refresh skill payments`
- the gh-compound-refresh skill performance-issues`
- the gh-compound-refresh skill critical-patterns`
...
<manual_override> Use the gh-compound skill [context] to document immediately without waiting for auto-detection. </manual_override> </auto_invoke>
...
- **Manual trigger**: User can invoke agents after the gh-compound skill completes for deeper review

## Related Commands

- the research skill [topic]` - Deep investigation (searches docs/solutions/ for patterns)
- the gh-plan skill - Planning workflow (references documented solutions)
...
If `gale-task` is not on PATH, skip silently — this must never block the skill.
<!-- the hkt-patch-gale-task-end skill -->
