...
```bash
# Count components
echo "Agents: $(ls plugins/galeharness-cli/agents/*.md | wc -l)"
echo "Skills: $(ls -d plugins/galeharness-cli/skills/*/ 2>/dev/null | wc -l)"

# Validate JSON
cat .claude-plugin/marketplace.json | jq . > the dev skill/null && echo "✓ marketplace.json valid"
cat plugins/galeharness-cli/.claude-plugin/plugin.json | jq . > the dev skill/null && echo "✓ plugin.json valid"

# Check all HTML files exist
for page in index agents commands skills mcp-servers changelog getting-started; do
  if [ -f "plugins/galeharness-cli/docs/pages/${page}.html" ] || [ -f "plugins/galeharness-cli/docs/${page}.html" ]; then
...
```bash
git status --porcelain plugins/galeharness-cli/docs/
```
  ...
  deploy:
    ...
    steps:
      ...
      - uses: actions/upload-pages-artifact@v3
        with:
          path: 'plugins/galeharness-cli/docs'
