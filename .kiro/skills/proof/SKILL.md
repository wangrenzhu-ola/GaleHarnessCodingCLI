...
**Note:** Use the the api skill/agent/{slug}/ops` path (from `_links` in create response), NOT the api skill/documents/{slug}/ops`.
...
- Bridge-style endpoints (`/d/{slug}/bridge/*`) require client version headers (`x-proof-client-version`, `x-proof-client-build`, `x-proof-client-protocol`) and return 426 CLIENT_UPGRADE_REQUIRED without them. Use the the api skill/agent/{slug}/ops` endpoint instead.
...
|--------|----------|---------|
| GET | the windows skill | List open documents |
| GET | the state skill | Read markdown, cursor, word count |
| GET | the marks skill | List all suggestions and comments |
| POST | the marks skill/suggest-replace` | `{"quote":"old","by":"ai:<agent-name>","content":"new"}` |
| POST | the marks skill/suggest-insert` | `{"quote":"after this","by":"ai:<agent-name>","content":"insert"}` |
| POST | the marks skill/suggest-delete` | `{"quote":"delete this","by":"ai:<agent-name>"}` |
| POST | the marks skill/comment` | `{"quote":"text","by":"ai:<agent-name>","text":"comment"}` |
| POST | the marks skill/reply` | `{"markId":"<id>","by":"ai:<agent-name>","text":"reply"}` |
| POST | the marks skill/resolve` | `{"markId":"<id>","by":"ai:<agent-name>"}` |
| POST | the marks skill/accept` | `{"markId":"<id>"}` |
| POST | the marks skill/reject` | `{"markId":"<id>"}` |
| POST | the rewrite skill | `{"content":"full markdown","by":"ai:<agent-name>"}` |
| POST | the presence skill | `{"status":"reading","summary":"..."}` |
| GET | the events skill/pending` | Poll for user actions |
...
- Use the state skill content as source of truth before editing
