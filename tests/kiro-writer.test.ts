...
describe("writeKiroBundle", () => {
  test("writes agents, skills, steering, and mcp.json", async () => {
    ...
    const bundle: KiroBundle = {
      ...
      steeringFiles: [
        { name: "galeharness-cli", content: "# Steering content\n\nFollow these guidelines." },
      ],
      mcpServers: {
    ...
    // Steering file
    const steeringPath = path.join(tempRoot, ".kiro", "steering", "galeharness-cli.md")
...
`,
    ...
    const installedSkill = await fs.readFile(
      path.join(tempRoot, ".kiro", "skills", "gh-plan", "SKILL.md"),
      "utf8",
    )
