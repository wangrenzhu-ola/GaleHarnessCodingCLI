...
const pluginRoot = path.join(process.cwd(), "plugins", "galeharness-cli")

describe("sanitizePathName", () => {
  test("replaces colons with hyphens", () => {
    expect(sanitizePathName("gh:brainstorm")).toBe("gh-brainstorm")
    expect(sanitizePathName("gh:plan")).toBe("gh-plan")
  })
