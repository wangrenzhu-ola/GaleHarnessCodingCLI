...
describe("convertClaudeToWindsurf dedupe", () => {
  test("agent skill deduplicates against sanitized pass-through skill names", () => {
    ...
    // The agent skill should get a deduplicated name since "gh:plan" normalizes
    // to "gh-plan" which collides with the pass-through skill on disk
    expect(bundle.agentSkills[0].name).not.toBe("gh-plan")
  })
})
