...
describe("release preview", () => {
  test("uses changed files to determine affected components and next versions", async () => {
    ...
    const preview = await buildReleasePreview({
      title: "fix: adjust gh:plan-beta wording",
      files: ["plugins/galeharness-cli/skills/gh-plan-beta/SKILL.md"],
    })

    expect(preview.components).toHaveLength(1)
    expect(preview.components[0].component).toBe("galeharness-cli")
    expect(preview.components[0].inferredBump).toBe("patch")
    expect(preview.components[0].nextVersion).toBe(bumpVersion(versions["galeharness-cli"], "patch"))
  })
