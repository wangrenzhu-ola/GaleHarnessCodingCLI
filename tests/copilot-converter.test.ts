...
describe("transformContentForCopilot", () => {
  test("rewrites .claude/ paths to .github/", () => {
    const input = "Read `.claude/galeharness-cli.local.md` for config."
    const result = transformContentForCopilot(input)
    expect(result).toContain(".github/galeharness-cli.local.md")
...
3. File at /tmp/output.md`
  ...
  test("transforms @agent references to agent references", () => {
    const input = "Have @security-sentinel and @gale-dhh-rails-reviewer check the code."
    const result = transformContentForCopilot(input)
    expect(result).toContain("the security-sentinel agent")
    expect(result).toContain("the gale-dhh-rails-reviewer agent")
    expect(result).not.toContain("@security-sentinel")
  })

  test("replaces ce: namespace with ce- in body text", () => {
    const input = "prefer gh:brainstorm first. Then run gh:plan and gh:review. Use ce:* skills."
    const result = transformContentForCopilot(input)
    expect(result).toBe("prefer gh-brainstorm first. Then run gh-plan and gh-review. Use ce-* skills.")
    expect(result).not.toContain("ce:")
  })

  test("replaces multi-colon ce: references fully", () => {
    const input = "run gh:work:beta and gh:review:deep"
    const result = transformContentForCopilot(input)
    expect(result).toBe("run gh-work-beta and gh-review-deep")
  ...
  test("ce: replacement does not corrupt URLs", () => {
    const input = "See https://example.com/gh:plan and http://docs.example.com/gh:review/overview"
    const result = transformContentForCopilot(input)
    expect(result).toContain("https://example.com/gh:plan")
    expect(result).toContain("http://docs.example.com/gh:review/overview")
  })
  ...
  test("generated skill deduplicates against sanitized pass-through skill names", () => {
    ...
    // since "gh:plan" and "gh-plan" both map to "gh-plan" on disk
    expect(bundle.generatedSkills[0].name).not.toBe("gh-plan")
  })
})
