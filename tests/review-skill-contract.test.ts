...
describe("ce-review contract", () => {
  test("documents explicit modes and orchestration boundaries", async () => {
    ...
    expect(content).toContain(".context/galeharness-cli/ce-review/<run-id>/")
  ...
  test("documents stack-specific conditional reviewers for the JSON pipeline", async () => {
    ...
    for (const agent of [
      "galeharness-cli:review:gale-dhh-rails-reviewer",
      ...
      "galeharness-cli:review:gale-julik-frontend-races-reviewer",
    ]) {
