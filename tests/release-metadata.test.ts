...
async function makeFixtureRoot(): Promise<string> {
  ...
  await mkdir(path.join(root, "plugins", "galeharness-cli", "agents", "review"), {
    recursive: true,
  })
  await mkdir(path.join(root, "plugins", "galeharness-cli", "skills", "gh-plan"), {
    recursive: true,
  })
  await mkdir(path.join(root, "plugins", "galeharness-cli", ".claude-plugin"), {
    recursive: true,
  })
  await mkdir(path.join(root, "plugins", "galeharness-cli", ".cursor-plugin"), {
    recursive: true,
  })
  ...
  await writeFile(
    path.join(root, "plugins", "galeharness-cli", "agents", "review", "agent.md"),
    "# Review Agent\n",
  )
  await writeFile(
    path.join(root, "plugins", "galeharness-cli", "skills", "gh-plan", "SKILL.md"),
    "# gh:plan\n",
  )
  await writeFile(
    path.join(root, "plugins", "galeharness-cli", ".mcp.json"),
    JSON.stringify({ mcpServers: { context7: { command: "ctx7" } } }, null, 2),
  )
  await writeFile(
    path.join(root, "plugins", "galeharness-cli", ".claude-plugin", "plugin.json"),
    JSON.stringify({ version: "2.42.0", description: "old" }, null, 2),
  )
  await writeFile(
    path.join(root, "plugins", "galeharness-cli", ".cursor-plugin", "plugin.json"),
    ...
    JSON.stringify(
      {
        ...
        plugins: [
          { name: "galeharness-cli", version: "2.41.0", description: "old" },
...
describe("release metadata", () => {
  ...
  test("detects cross-surface version drift even without explicit override versions", async () => {
    ...
    expect(changedPaths).toContain(path.join(root, "plugins", "galeharness-cli", ".cursor-plugin", "plugin.json"))
