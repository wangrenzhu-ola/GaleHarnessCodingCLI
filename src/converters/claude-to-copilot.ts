...
export function transformContentForCopilot(body: string): string {
  ...
  // 1. Transform Task agent calls (supports namespaced names like galeharness-cli:research:agent-name)
  ...
  // 3. Replace plugin colon-namespaced command references (e.g. gh:plan → gh-plan, ce:plan → ce-plan)
  // Supports both gh: (GaleHarnessCLI) and ce: (compound-engineering legacy) prefixes.
  ...
  // Captures colons in the name segment so multi-colon refs like gh:work:beta → gh-work-beta.
  result = result.replace(/(?<=^|[\s,.()`'"])(gh|ce):([a-z*][a-z0-9_*:-]*)/gim, (_, prefix: string, name: string) => `${prefix}-${name.replace(/:/g, "-")}`)
