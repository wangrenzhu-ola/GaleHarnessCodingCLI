...
export async function getCompoundEngineeringCounts(root: string): Promise<CompoundEngineeringCounts> {
  const pluginRoot = path.join(root, "plugins", "galeharness-cli")
...
export async function syncReleaseMetadata(options: SyncOptions = {}): Promise<MetadataSyncResult> {
  ...
  const compoundClaudePath = path.join(root, "plugins", "galeharness-cli", ".claude-plugin", "plugin.json")
  const compoundCursorPath = path.join(root, "plugins", "galeharness-cli", ".cursor-plugin", "plugin.json")
  ...
  for (const plugin of marketplaceClaude.plugins) {
    if (plugin.name === "galeharness-cli") {
