...
function resolveBundledMcporterConfigPath(): string | undefined {
  try {
    ...
    const candidates = [
      path.join(extensionDir, "..", "pi-resources", "galeharness-cli", "mcporter.json"),
      path.join(extensionDir, "..", "galeharness-cli", "mcporter.json"),
    ]
...
function resolveMcporterConfigPath(cwd: string, explicit?: string): string | undefined {
  ...
  const projectPath = path.join(cwd, ".pi", "galeharness-cli", "mcporter.json")
  if (fs.existsSync(projectPath)) return projectPath

  const globalPath = path.join(os.homedir(), ".pi", "agent", "galeharness-cli", "mcporter.json")
