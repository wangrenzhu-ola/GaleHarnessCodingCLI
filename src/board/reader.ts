...
const DEFAULT_DB_PATH = join(homedir(), ".galeharness", "tasks.db")
...
export async function readAndMergeTasks(overrideDbPath?: string): Promise<DerivedTask[]> {
  const dbPath = overrideDbPath ?? DEFAULT_DB_PATH
...
export function getDbPath(): string {
  return process.env.GALE_TASKS_DB_PATH ?? DEFAULT_DB_PATH
}
