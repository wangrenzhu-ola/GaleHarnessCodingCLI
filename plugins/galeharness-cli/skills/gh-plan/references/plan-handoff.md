...
**Options:**
1. **Start `/gh:work`** - Begin implementing this plan in the current environment (recommended)
...
5. **Start `/gh:work` in another session** - Begin implementing in a separate agent session when the current platform supports it
...
- **`/gh:work`** -> Call `/gh:work` with the plan path
- **`/gh:work` in another session** -> If the current platform supports launching a separate agent session, start `/gh:work` with the plan path there. Otherwise, explain the limitation briefly and offer to run `/gh:work` in the current session instead.
...
- Ask whether to proceed to `/gh:work`
