...
This content is loaded when a synthesis-summary phase fires in gh-plan. There are two variants — they share structure but differ in timing and content focus:
...
- AND Phase 0.4 stayed in gh-plan (did not route to gh:debug, gh:work, or universal-planning)
...
Does this match your intent? Tell me what to add, remove, redirect, or that I got wrong — or just confirm to proceed. (You can also redirect to /gh:brainstorm if this is bigger than you initially thought — I'll stop here and load it for you.)
```
...
When the skill is invoked from an automated workflow such as LFG or any `disable-model-invocation` context, the skill runs in non-interactive mode (no synchronous user). The artifact is read by downstream skills (document-review, gh:work) and human reviewers (PR review).
...
This restores the audit visibility the original design intended (un-validated bets must not propagate as authoritative content), but surfaces them under their own label rather than hiding them. Downstream review (document-review, gh:work, human PR review) can scrutinize Assumptions specifically.
...
- **Solo variant**: common redirects include "this is bigger than I thought — let me brainstorm first" (suggest `/gh:brainstorm`), "this is just a fix, no plan needed" (suggest `/gh:work`), or "I need to investigate first" (suggest `/gh:debug`).
- **Brainstorm-sourced variant**: less common, but possible — "actually this scope is wrong, take it back to brainstorm" (suggest `/gh:brainstorm` to revise the upstream doc).

In either case: stop gh-plan, suggest the alternative skill, offer to load it in-session. Don't push back or argue — the user's redirect signal is the deliberate choice.
