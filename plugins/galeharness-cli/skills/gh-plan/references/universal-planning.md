...
- **Pipeline mode?** If invoked from LFG or any `disable-model-invocation` context: output "This is a non-software task. The LFG pipeline requires gh:work, which only supports software tasks. Use `/gh:plan` directly for non-software planning." and stop.
