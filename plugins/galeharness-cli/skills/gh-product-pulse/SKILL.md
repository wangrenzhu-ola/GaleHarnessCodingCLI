...
`gh:product-pulse` queries the product's data sources for a given time window and produces a compact, single-page report covering usage, performance, errors, and followups. The report is saved to `docs/pulse-reports/` and the key points are surfaced in chat.

The skill does not mutate the product, the database, or any external system. Its only writes are pulse settings appended to `.compound-engineering/config.local.yaml` (the unified GaleHarness local config, gitignored, machine-local) and the report file (`docs/pulse-reports/...`). MCP and other data-source tools are invoked read-only; if a tool offers write modes, do not use them.
...
Use `STRATEGY.md`, repo docs, issues, PRs, and read-only product data as fact sources. Gale-managed HKTMemory (`gale-memory`) can help recall prior pulse findings, but saved reports in `docs/pulse-reports/` and durable project artifacts are the source of truth. Never persist PII to HKTMemory or pulse reports.
...
If `STRATEGY.md` does not exist, note that explicitly in chat: no strategy doc on file, running setup from scratch, and mention that `gh:strategy` can seed pulse later if run first.
