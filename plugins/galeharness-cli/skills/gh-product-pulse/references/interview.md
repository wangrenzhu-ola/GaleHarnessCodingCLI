...
Loaded by `SKILL.md` at the start of Phase 1. Captures the configuration that will be merged into `.compound-engineering/config.local.yaml` (the unified GaleHarness local config, gitignored, machine-local) as `pulse_*` keys and re-read on every subsequent run.
