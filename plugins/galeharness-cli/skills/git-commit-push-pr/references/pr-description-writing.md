...
How to resolve the right commit range and compose a PR title and body. Loaded on demand by `git-commit-push-pr` callers — do not load earlier.
...
- **Current-branch mode.** Describe HEAD vs the repo's default base. Used when the caller has no explicit PR reference (Step 6 of git-commit-push-pr's full workflow; description-only mode without a PR ref).
...
2. **No existing evidence block:** omit the evidence section entirely unless the caller already captured evidence and passed it in (e.g., a `gh-demo-reel` URL).

Do not label test output as "Demo" or "Screenshots". Place any preserved evidence block before the GaleHarness CLI badge.
...
- **No Review / process section**: Do not include a section describing how the reviewer should review (checklists of things to look at, process bullets). Process doesn't help the reviewer evaluate code. Call out specific non-obvious things to scrutinize inline with the relevant change.
...
5. **GaleHarness CLI badge** -- append a badge footer separated by a `---` rule. Skip if regenerating an existing body that already contains the badge.
...
[![GaleHarness CLI](https://img.shields.io/badge/Built_with-GaleHarness_CLI-6366f1)](https://github.com/wangrenzhu-ola/GaleHarnessCodingCLI)
