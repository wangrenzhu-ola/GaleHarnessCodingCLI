...
`gh:strategy` produces and maintains `STRATEGY.md` - a short, durable anchor document that captures what the product is, who it serves, how it succeeds, and where the team is investing. It lives at the repo root as a canonical, well-known file (peer of `README.md`). Downstream GaleHarness skills (`gh:ideate`, `gh:brainstorm`, `gh:plan`) read it as grounding when it exists.
...
## GaleHarness Knowledge Conventions

`STRATEGY.md` is the durable project fact source. HKTMemory/gale-memory may be used for recall, but do not treat recalled memory as authoritative unless it is read back from repo files, issues, PRs, or other durable project sources. If the repo has Gale-managed public knowledge under `~/.galeharness/knowledge/<project>/hkt-memory`, use it only to discover candidate context and then verify against durable files before writing strategy content.

## Core Principles

1. **Anchor, not plan.** Strategy is what the product is and why. Features belong in `gh:brainstorm`; schedules belong in the issue tracker. Do not let either creep into the doc.
...
After writing, note in one line where the file lives and that `gh:ideate`, `gh:brainstorm`, and `gh:plan` will pick it up as grounding on their next run.

If no downstream skill has run yet on this repo, suggest `gh:ideate` or `gh:brainstorm` skills as a next step.
...
- Does not write product requirements or implementation plans - those are `gh:brainstorm` and `gh:plan`.
