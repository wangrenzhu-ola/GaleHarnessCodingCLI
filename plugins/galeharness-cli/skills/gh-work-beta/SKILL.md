...
**Config (pre-resolved):**
!`cat "$(git rev-parse --show-toplevel 2>/dev/null)/.galeharness-cli/config.local.yaml" 2>/dev/null || cat "$(dirname "$(git rev-parse --path-format=absolute --git-common-dir 2>/dev/null)")/.galeharness-cli/config.local.yaml" 2>/dev/null || echo '__NO_CONFIG__'`
...
If it shows an unresolved command string, read `.galeharness-cli/config.local.yaml` from the repo root using the native file-read tool (e.g., Read in Claude Code, read_file in Codex). If the file does not exist, all settings fall through to defaults.
