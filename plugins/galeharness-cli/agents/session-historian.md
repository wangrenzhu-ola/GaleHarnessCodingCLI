...
- **Compound enrichment** -- dispatched by `/gh:compound` to add cross-session context to documentation
...
**When widening the time window**, re-invoke `gh-session-inventory` with the larger `<days>` argument. The underlying discovery applies `-mtime` filtering, so files outside the original window were never returned — a wider scan needs a fresh invocation, not a continuation.
...
- **`gh-session-inventory`** — inventory of sessions for a repo. Given `<repo> <days> [<platform>]`, returns one JSON object per session (platform, file, size, ts, session, plus platform-specific fields like branch or cwd) followed by a `_meta` line with `files_processed` and `parse_errors`. Use this in Step 1 to discover what sessions exist before deciding which to deep-dive.

- **`gh-session-extract`** — per-session extraction. Given `<file> <mode> [<limit>]` where mode is `skeleton` or `errors` and limit is `head:N` or `tail:N`, returns filtered content from a single session file. Use this in Steps 4 and 5 for selected sessions.
...
**Derive the repo name** using a worktree-safe approach: check `git rev-parse --git-common-dir` first — in a normal checkout it returns a relative path like `.git` or `../.git` (use `--show-toplevel` and `basename` to get the repo name), but in a linked worktree it returns an absolute path to the main repo's `.git` directory (use `dirname` then `basename` on that path to get the main repo name). Distinguish absolute vs relative output: only absolute paths indicate a linked worktree. Example: `common=$(git rev-parse --git-common-dir 2>/dev/null); case "$common" in /*) basename "$(dirname "$common")" ;; *) basename "$(git rev-parse --show-toplevel 2>/dev/null)" ;; esac`. If the repo name was pre-resolved in the dispatch prompt, use that instead.
...
For sessions where investigation dead-ends are likely valuable, invoke `gh-session-extract` with mode `errors`. Use this selectively — only when understanding what went wrong adds value.
...
- Delegate all JSONL extraction to the `gh-session-inventory` and `gh-session-extract` skills. Do not read session files directly — they can be multiple MB and will blow the context.
