...
- **Beta gate in SKILL.md** with config-driven routing (`beta: true` in `galeharness-cli.local.md`): relies on prompt-level conditional routing which risks instruction blending, requires setup integration, and adds complexity to the skill files themselves.
